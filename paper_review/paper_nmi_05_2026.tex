...
Section~\ref{sec:background} establishes why monitoring agent tools can anticipate risks of AI agents. Section~\ref{sec:data} describes our panel data on tools and their use. Section~\ref{sec:results} presents findings, Section~\ref{sec:discussion} discusses risk implications, and Section~\ref{sec:conclusions} concludes. Methods (Section~\ref{sec:methodology}) at the end of the paper detail classification and validation.
...
The dataset of 177,436 MCP tools curated for this study is available from the corresponding author (merlin.stein@bsg.ox.ac.uk) on reasonable request. Public release of the full dataset is constrained by upstream licensing: PyPI metadata is distributed under the CC-BY 4.0 licence, but a substantial fraction of the GitHub repositories from which we collect README content do not carry an explicit licence, which prevents us from redistributing their text. Aggregate statistics, per-server classifications, and the list of public repository URLs covered by the analysis can be shared with referees during review and with other researchers post-publication on request.

\section*{Code availability}

Code for replicating the analyses reported in this paper is available from the corresponding author on reasonable request. We do not yet make the codebase public to allow further consolidation; classification prompts (Appendix~\ref{app:prompts}) and methodology (Methods, Section~\ref{sec:methodology}) are documented in sufficient detail to permit independent re-implementation.
