...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('conseq_fin_stage4_dataenrichment.log'),
...
def enrich_stage4_with_metadata(
    stage4_path: str = "conseq_fin_stage4_task_output.csv",
    stage2_path: str = "conseq_fin_stage2.csv",
    usage_data_path: str = "data_usage.json",
    output_path: Optional[str] = None
) -> str:
    """
    Enrich stage4 task output CSV with creation_date and use_count from stage2 CSV,
    plus usage data from data_usage.json.
    
    Args:
        stage4_path: Path to the stage4 task output CSV file
    ...
    # Read stage4 data
    logger.info("Loading stage4 task output data...")
    try:
        stage4_df = pd.read_csv(stage4_path)
        logger.info(f"Loaded {len(stage4_df)} rows from stage4 file")
    except Exception as e:
        logger.error(f"Error reading stage4 file: {e}")
        raise
    
    # Validate stage4 has required columns
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enrich stage4 task output with creation dates, use counts from stage2, and usage data"
    )
    parser.add_argument(
        '--stage4', 
        default="conseq_fin_stage4_task_output.csv",
        help="Path to stage4 task output CSV file"
    )
    parser.add_argument(
        '--stage2', 
        default="conseq_fin_stage2.csv",
    ...
    try:
        output_file = enrich_stage4_with_metadata(
