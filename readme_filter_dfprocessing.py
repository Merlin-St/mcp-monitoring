...
Processes Inspect evaluation results from readme_filter_inspect.py and updates
...
Usage:
    ...
    python readme_filter_dfprocessing.py --logs-dir ./logs
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('readme_filter_dfprocessing.log'),
...
def load_eval_results(logs_dir: str) -> List[Dict[str, Any]]:
    ...
    try:
        # Use Inspect's DataFrame processing functions
        from inspect_ai.analysis.beta import samples_df, messages_df
        
        samples_df_data = samples_df(logs_dir)
        messages_df_data = messages_df(logs_dir)
...
def process_evaluation_results(eval_results: List[Dict[str, Any]], 
    ...
    for result in eval_results:
        ...
        # Extract filtered content (raw_output is the LLM response)
        filtered_content = result.get('raw_output', '')
        ...
        if server_index is not None:
            ...
            # Update server with Stage 2 filtered content
...
def main():
    ...
    summary_file = 'readme_filter_dfprocessing_summary.json'
