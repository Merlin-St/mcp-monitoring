...
- ONET task data: `conseq_fin_stage4_onet_taskstatements.csv`
...
### Primary Output
- **`conseq_fin_stage4_tasks_cluster_names.csv`** - Complete task assignments with cluster names
  ```
  task_id | onet_code | task | title | level2_cluster | level2_name | level1_cluster | level1_name
  ```

### Summary
- **`conseq_fin_stage4_task_summary.json`** - Pipeline statistics and validation scores
  ```json
  {
