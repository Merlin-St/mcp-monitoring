...
When "Nearby tasks from other clusters" are provided, use them as context to understand what distinguishes this cluster from its neighbors, but focus your name on the tasks within the main cluster.
...
def prepare_cluster_naming_samples(clusters_info: Dict[str, Dict[str, Any]], level: str = 'level2') -> List[Sample]:
    ...
    for cluster_identifier, info in clusters_info.items():
        if level == 'level2':
            ...
            cluster_tasks = [f"- {task}" for task in info['tasks']]
            prompt_parts = ["\n".join(cluster_tasks)]
            
            # Add boundary tasks if available (contrastive mode)
            if 'boundary_tasks' in info and info['boundary_tasks']:
                boundary_tasks = [f"- {task}" for task in info['boundary_tasks']]
                prompt_parts.extend([
                    "\nNearby tasks from other clusters (for context):",
                    "\n".join(boundary_tasks)
                ])
...
def l3_to_l2_validation():
    ...
    return Task(
        dataset=json_dataset("data/internal-task-clusters/task_clusters_validation_l3_to_l2.jsonl"),
        solver=[
...
def l2_to_l1_validation():
    ...
    return Task(
        dataset=json_dataset("data/internal-task-clusters/task_clusters_validation_l2_to_l1.jsonl"),
        solver=[
            system_message(VALIDATION_SYSTEM_PROMPT),
            generate()
        ],
...
def l3_to_l1_validation():
    ...
    return Task(
        dataset=json_dataset("data/internal-task-clusters/task_clusters_validation_l3_to_l1.jsonl"),
        solver=[
            system_message(VALIDATION_SYSTEM_PROMPT),
            generate()
        ],
...
def subset_l2_l3_validation():
    ...
    return Task(
        dataset=json_dataset("data/internal-task-clusters/task_clusters_validation_subset_l2_l3.jsonl"),
        solver=[
            system_message(VALIDATION_SYSTEM_PROMPT),
            generate()
        ],
