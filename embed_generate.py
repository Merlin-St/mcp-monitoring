...
# OPTIMIZATION 2: Implement proper batching and mixed precision
def generate_high_quality_embeddings(texts, model_name='Qwen/Qwen3-Embedding-0.6B', 
                                    device='cuda', batch_size=256, cache_dir='embeddings_cache'):
    """
    Converts a list of texts into high-quality numerical embeddings using the Qwen model.
    ...
    with tqdm(total=num_batches, desc="Generating embeddings", unit="batch") as pbar:
        for i in range(0, len(texts), batch_size):
            ...
            if has_gpu and i % (batch_size * 10) == 0:
                torch.cuda.empty_cache()
...
def create_bertopic_model(embeddings, texts, min_cluster_size=25, n_neighbors=15):
    ...
    # Optimized vectorizer with reduced vocabulary
...
def create_interactive_plot(df, topic_info=None):
    ...
    fig = px.scatter(
        ...
        title='BERTopic Analysis of MCP Server Descriptions (Optimized Qwen-0.6B)',
...
def main():
    ...
    if not df.empty and embeddings is not None:
        ...
        n_neighbors = min(15, len(df) - 1) if args.test else 15
        ...
        print(f"Check {html_filename} for visualization.")
    else:
