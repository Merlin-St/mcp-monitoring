...
from conseq_fin_stage4_task_clusters_llm import (
...
def get_cluster_info_with_boundaries(
    df: pd.DataFrame, 
    task_embeddings: np.ndarray, 
    level: str = 'level2',
    boundary_tasks: int = 8
) -> Dict[str, Dict[str, Any]]:
    """
    Get information about clusters for LLM naming, including boundary tasks
    
    Following the research specification: "For each neighborhood, use Claude to propose
    candidate higher-level task descriptions by examining both the tasks within the neighborhood
    and the nearest m tasks outside it."
    
    Args:
        df: DataFrame with cluster assignments
        task_embeddings: Embeddings for all tasks (aligned with df rows)
        level: 'level2' or 'level1' for different levels
        boundary_tasks: Number of nearest tasks outside each cluster to include
        
    Returns:
        Dict mapping cluster identifier to cluster info including sample tasks and boundary tasks
    """
    logger.info(f"Getting cluster info with {boundary_tasks} boundary tasks per cluster")
    cluster_col = f'{level}_cluster'
    clusters = {}
    
    # Verify embeddings alignment
    if len(df) != len(task_embeddings):
        logger.error(f"Embedding alignment error: {len(df)} tasks vs {len(task_embeddings)} embeddings")
        raise ValueError("Task embeddings must align with DataFrame rows")
    
    for cluster_identifier in sorted(df[cluster_col].unique()):
        cluster_tasks = df[df[cluster_col] == cluster_identifier]
        cluster_indices = cluster_tasks.index.tolist()
        
        # Get tasks outside this cluster
        outside_cluster = df[df[cluster_col] != cluster_identifier]
        outside_indices = outside_cluster.index.tolist()
        
        if len(outside_indices) == 0:
            logger.warning(f"No tasks outside cluster {cluster_identifier}")
            boundary_task_texts = []
        else:
            # Calculate similarities between cluster center and all outside tasks
            cluster_embeddings = task_embeddings[cluster_indices]
            outside_embeddings = task_embeddings[outside_indices]
            
            # Use cluster centroid for boundary task selection
            cluster_center = np.mean(cluster_embeddings, axis=0).reshape(1, -1)
            similarities = cosine_similarity(cluster_center, outside_embeddings)[0]
            
            # Find nearest boundary tasks
            nearest_indices = np.argsort(similarities)[-boundary_tasks:][::-1]  # Top similar, descending
            nearest_outside_indices = [outside_indices[i] for i in nearest_indices]
            
            boundary_task_texts = df.loc[nearest_outside_indices, 'Task'].tolist()
        ...
        clusters[cluster_identifier] = {
            ...
            'tasks': cluster_tasks['Task'].tolist(),
            'sample_tasks': cluster_tasks.sample(min(10, len(cluster_tasks)), random_state=42)['Task'].tolist(),
            'boundary_tasks': boundary_task_texts
        }
    
    logger.info(f"Generated cluster info for {len(clusters)} clusters with boundary tasks")
    return clusters