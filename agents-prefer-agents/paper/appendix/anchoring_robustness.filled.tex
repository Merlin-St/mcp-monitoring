...
\paragraph{Result.} Cross-stratum DiD ($B - A$) $= +2.28{}$~pp (95\% CI [+1.31, +3.26]{}). AI's prior review nudges humans toward approving AI-authored PRs by less than one percentage point relative to the no-AI-reviewer baseline. The within-PR DiD reported in \S\ref{sec:results} ($\widehat{\rbt^A - \rbt^H} = -14.17{}$~pp) is more than an order of magnitude larger than this anchoring effect, and the two CIs do not overlap. Anchoring is therefore not the main driver of the headline result.
